...
If a token is successfully validated, the `orgno`, `clientPid`, and `consentingPid` request attributes are set according to the claims supplied in the token, and may be bound in your controller using the Spring `@RequestAttribute` annotation.
...
| maskinporten.validation.requireConsent (optional) | Whether to require a authorization_details claim in all received Maskinporten tokens. Default: False.                                 |
